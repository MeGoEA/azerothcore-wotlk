...
bool WorldSession::ChannelCheck(std::string channel)
{
	...
	if (channel.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789. АаБбВвГгДдЕеЁёЖжЗзИиЙйКкЛлМмНнОоПпРрСсТтУуФфХхЦцЧчШшЩщЪъЫыЬьЭэЮюЯя") != std::string::npos)
		return false;
...
void WorldSession::HandleJoinChannel(WorldPacket& recvPacket)
{
    ...
    if (channelName.find("|") != std::string::npos || !utf8::is_valid(channelName.begin(), channelName.end()))
        return;
...
void WorldSession::HandleChannelSetOwner(WorldPacket& recvPacket)
{
    ...
    recvPacket >> channelName >> targetName;
		...
		if (!ChannelCheck(channelName) || !ChannelCheck(targetName))
		return;
